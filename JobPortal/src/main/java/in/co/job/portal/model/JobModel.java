...
public class JobModel {
	...
	public long add(JobBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			System.out.println(pk + " in ModelJDBC rizi");
	...
	public JobBean findByPK(long pk) throws ApplicationException {
		...
		try {
			...
			if (!rs.isBeforeFirst() ) {    
				bean = new JobBean();
				bean.setDescription("NO DISCRIPTION FOUND");
			...
			while (rs.next()) {
				...
				bean.setCreatedDatetime(rs.getTimestamp(10));
				bean.setModifiedDatetime(rs.getTimestamp(11));
				bean.setProcessed(rs.getString(12)+"");
			}
			rs.close();
		}
			} catch (Exception e) {
	...
	public long jobadd(JobBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			// Get auto-generated next primary key
			System.out.println(pk + " in ModelJDBC rizi2");
			conn.setAutoCommit(false); // Begin transaction
	...
	public void updateJobStatus(int id) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			conn.setAutoCommit(false); // Begin transaction
			PreparedStatement pstmt = conn.prepareStatement(
					"UPDATE p_job SET Processed=? WHERE ID= (select jobid from p_applay where id = ?)");
			pstmt.setString(1, "Y");
			pstmt.setLong(2, id);
			pstmt.executeUpdate();
			conn.commit(); 
