	...
	protected boolean validate(HttpServletRequest request) {
		...
		if (DataValidator.isNull(request.getParameter("userType"))) {
			request.setAttribute("userType",
					PropertyReader.getValue("error.require", "User Type"));
			pass = false;
		}
...
//		bean.setRoleId(3L);
		...
		if(!(null==DataUtility.getString(request.getParameter("userType"))) && DataUtility.getString(request.getParameter("userType")).equalsIgnoreCase("CANDIDATE")) {
			bean.setRoleId(3L);
		}else {
			bean.setRoleId(2L);
		}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (OP_SIGN_UP.equalsIgnoreCase(op)) {
			...
			//	System.out.println("in try sign up");
				...
				ServletUtility.forward(JPView.USER_REGISTRATION_VIEW, request, response);
				return;
			...
			ServletUtility.redirect(JPView.USER_REGISTRATION_CTL, request, response);
			return;
		}
	...
	protected String getView() {
		return JPView.USER_REGISTRATION_VIEW;
	}
